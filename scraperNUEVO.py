...
ACTOR_ID = os.getenv("ACTOR_ID", "scrapestorm/google-news-scraper-fast-cheap-pay-per-results")
...
for query in QUERIES:
    for country in COUNTRIES:
        country_label = COUNTRY_LABEL_MAP.get(country, country)
        run_input = {
            "keywords":             [query],
            "timeFilter":           os.getenv("TIME_FILTER", "Less than a day"),
            "language":             "Spanish",
            "country":              country_label,
            "maxResultsPerKeyword": MAX_ITEMS,
        }
...
# ── Normalizar columnas del nuevo actor → nombres que usa el resto del script ──
# Campos que devuelve scrapestorm/google-news-scraper-fast-cheap-pay-per-results:
#   "Id", "Keyword", "Title", "Description", "Source Name",
#   "Published_time", "Date", "Link", "Image"
COLUMN_RENAME = {
    "Link":        "link",
    "Title":       "title",
    "Description": "snippet",
    "Source Name": "source",
    "Date":        "date_utc",   # "2026-05-15 14:44:24"
}
final_df.rename(columns=COLUMN_RENAME, inplace=True)

# 'domain' se deriva de 'source' (el actor no devuelve dominio separado)
if "domain" not in final_df.columns:
    final_df["domain"] = final_df.get("source", pd.Series("", index=final_df.index))

# Limpiar &nbsp; del snippet
if "snippet" in final_df.columns:
    final_df["snippet"] = final_df["snippet"].astype(str).str.replace(r"&nbsp;", " ", regex=False).str.strip()
...
final_df['country'] = final_df['country'].replace(COUNTRY_NAME_MAP)
